...
return inquirer
  ...
  .then((response) => {
    let shape;
    let shapeType; ['circle', 'triangle', 'shape']
    ...
    const svgData = `<svg xmlns="http://www.w3.org/2000/svg" width="300" height="200"><shape="${response.shape}"/><fill="${response.shapeColor}"/><text="${response.textColor}"></svg>`;
